...
class BaseStation:
    ...
    def step(self, current_time):
        ...
        # 6. 记录轨迹
        #当前哪个UE在传输
        self.BS_obs = np.array([self.current_transmitting]) if self.current_transmitting is not None else np.array([0])
...
def run_simulation(num_ues, buffer_size, arrival_prob, num_steps):
    ...
    for _ in range(repititions):
        ...
        # 保存数据集为JSON
        with open(f'signaling_dataset_UE{num_ues}.json', 'w') as f:
